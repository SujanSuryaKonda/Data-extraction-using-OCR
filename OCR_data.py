...
icon = Image.open("'''address to any business card''")
st.set_page_config(page_title="BizCardX: Extracting Business Card Data with OCR | By Sujan Surya Konda",
                   page_icon=icon,
                   layout="wide",
                   initial_sidebar_state="expanded",
                   menu_items={'About': """# This OCR app is created by *Sujan Surya Konda*!"""})
...
def setting_bg():
    st.markdown(f""" <style>.stApp {{
                        background:"````back-ground image adresss```" );
...
#mycursor.execute("create database ocr_db")
...
if selected == "Home":
    ...
    with col2:
        st.image("```back ground image address")
